% This program bootstraps a bivariate VAR
...
for j=1:J
    j
    eps = random('Normal',0,1,T+K,2)*A0;
    ylong = zeros(T+K,2); %initialize the time series
    ...
    Sighat = (epshat'*epshat)/(T-(2*K+1));
    beta0hat = betahat(1,:);
    betaLhat = zeros(2,2,K);
    for k=1:K
        betaLhat(:,:,k) = betahat(2*k:2*k+1,:);
    end
    ...
    for n=1:N
        ...
        %constuct y according to specified AR process
        ylongnew = zeros(T+K,2); %initialize the time series
        for t=K+1:T+K
        ...
        %construct var-covar matrix
        Sigtilde1 = (epstilde'*epstilde)/(T-(2*K+1)); %wrong way
        Sigtilde2 = (epstilde'*epstilde)*(T/(T-(2*K+1))^2); %right way
